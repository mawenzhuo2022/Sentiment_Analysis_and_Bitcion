...
class FinancialPredictor:
    """
    LSTM training and inference workflow tailored for financial time series like Bitcoin prices.

    Attributes:
        target_column (str): Name of the column to forecast.
        scaler (MinMaxScaler): Scaler for normalizing features.
        model (keras.Model | None): Trained LSTM model instance.
        history (History | None): Keras training history object.
    """
    ...
    def prepare_data(self, data, features, lookback=1):
        """
        Build sliding windows for LSTM input while handling missing values and scaling.

        Args:
            data (pd.DataFrame): Source dataframe with time-series features.
            features (list[str]): Columns to use as inputs.
            lookback (int): Sequence length for the LSTM window.

        Returns:
            tuple[np.ndarray, np.ndarray]: LSTM input tensor (samples, lookback, features)
            and the aligned target vector.
        """
    ...
    def create_model(self, input_shape, lstm_units=50, dropout_rate=0.2, learning_rate=0.001):
        """
        Construct the base LSTM sequential model.

        Args:
            input_shape (tuple[int, int]): Shape of the input tensor (lookback, feature_dim).
            lstm_units (int): Hidden units per LSTM layer.
    ...
    def grid_search(self, X_train, y_train, X_val, y_val, feature_columns):
        """
        Perform a lightweight grid search over hyperparameters.
        ...
        Args:
            ...
            feature_columns (list[str]): Column names used to infer feature dimension.

        Returns:
            dict: Best combination for lstm_units, dropout_rate, learning_rate.
        """
        param_grid = {
            'lstm_units': [32, 50, 64],
    ...
    def train(self, X_train, y_train, feature_columns, epochs=30, batch_size=32, validation_split=0.1,
        ...
        """
        Train the LSTM model with early stopping for stability.

        Args:
            X_train (np.ndarray): Training inputs.
            y_train (np.ndarray): Training targets.
            feature_columns (list[str]): Feature names to determine input dimension.
    ...
    def evaluate(self, y_true, y_pred):
        """
        Invert scaling and compute regression metrics (MSE, MAE, R²).

        Args:
            y_true (np.ndarray): Ground-truth values in normalized space.
            y_pred (np.ndarray): Predicted values prior to inverse transform.

        Returns:
            dict[str, float]: Metric dictionary for MSE, MAE, R2.
        """
    ...
    def plot_results(self, y_true, y_pred, dates, title, lookback):
        """
        Plot actual vs predicted series and print summary statistics.

        Args:
            y_true (np.ndarray): Ground-truth values in normalized space.
            y_pred (np.ndarray): Predicted values.
            dates (np.ndarray): Timeline used for plotting.
            title (str): Figure title.
            lookback (int): Window size for aligning timestamps.
        """
    ...
    def predict(self, X_test):
        """
        Run inference on the test set and transform back to the original scale.

        Args:
            X_test (np.ndarray): Test input tensor.

        Returns:
            np.ndarray: Inverted predictions with shape (samples, 1).
        """
    ...
    def plot_training_history(self):
        """
        Visualize training/validation loss and loss improvement trends for diagnostics.
        """
...
def train_and_evaluate(data, target_column, feature_columns, lookback=5, epochs=30, do_grid_search=False):
    """
    End-to-end helper covering splits, training, evaluation, and visualization.

    Args:
        data (pd.DataFrame): Source dataset containing all features.
        target_column (str): Target column name.
        feature_columns (list[str]): Feature names for model input.
        lookback (int): LSTM window size.
        epochs (int): Training epochs.
        do_grid_search (bool): Whether to perform grid search for hyperparameters.

    Returns:
        tuple[FinancialPredictor, dict, np.ndarray]: Predictor, metric dict, and test dates.
    """
...
def try_different_lookbacks(data, target_column, feature_columns, lookbacks=[3, 5, 7, 10], epochs=30,
    ...
    """
    Iterate through multiple lookback values to compare performance empirically.

    Args:
        data (pd.DataFrame): Full dataset.
        target_column (str): Target column name.
        feature_columns (list[str]): Feature names used as inputs.
        lookbacks (list[int]): Candidate window sizes.
        epochs (int): Training epochs for each experiment.
        do_grid_search (bool): Whether to run grid search per configuration.

    Returns:
        tuple[dict, int]: Mapping of lookback to results and the best lookback.
    """
    results = {}
    best_lookback = None
    best_mse = float('inf')
